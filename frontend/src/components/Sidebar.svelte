<script>
  ...
  import GreylineTab from './GreylineTab.svelte';
...
<div class="bg-slate-800/50 backdrop-blur rounded-lg border border-slate-700/50 flex flex-col h-full" style="height: 100%; max-height: 100%;">
  ...
  <div class="flex border-b border-slate-700/50 bg-slate-900/30 flex-shrink-0 overflow-x-auto">
    ...
    <button
      class="px-3 py-2 text-sm font-semibold transition-colors whitespace-nowrap {activeTab === 'greyline' ? 'bg-amber-500/20 text-amber-400 border-b-2 border-amber-500' : 'text-slate-400 hover:text-slate-300'}"
      on:click={() => activeTab = 'greyline'}>
      <svg class="w-4 h-4 inline-block mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  <div class="flex-1 overflow-hidden" style="min-height: 0;">
    ...
    {:else if activeTab === 'log'}
      <LogTab {recentQSOs} {logStats} {dxccProgress} {huntStatus} />
